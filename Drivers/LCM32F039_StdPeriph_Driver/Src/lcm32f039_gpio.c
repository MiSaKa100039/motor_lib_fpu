/**
  ...
  * @version V2.0.4
  * @date    2025-03-18
  * @brief   This file provides all the gpio emulation firmware functions.
  ******************************************************************************/
/**
  ******************************************************************************
  * @attention:
...
void GPIO_ResetPinConfig(uint32_t NewState)
{
  ...
  {
    chipctrl_access();
    CHIPCTRL->BDCR_b.RST_CFG = 1; // PF2?????¦Ë???? /PF2 serves as the reset pin
    __dekey();
  }
}
/**
...
void GPIO_Init(GPIO_TypeDef *GPIOx, GPIO_InitTypeDef *GPIO_InitStruct)
{
  ...
  pinpos = GPIO_InitStruct->GPIO_Pin; // ????§Ø???¦Ë????? /Fetch how many bits to configure
  ...
  for (pos = 0; pos < 16; pos++) // ???§Ò?? /Flip the slope
  {
    if ((pinpos >> pos) & 0x01)
    {
  ...
  for (pos = 0; pos < 16; pos++) // ??????? /The output type
  {
    if ((pinpos >> pos) & 0x01)
    {
      if (GPIO_InitStruct->GPIO_OType == GPIO_OType_OD)
      {
				GPIOx->OD_BSRR = (1 << pos); // open drain enable
      }
      else if (GPIO_InitStruct->GPIO_OType == GPIO_OType_OS)
      {
