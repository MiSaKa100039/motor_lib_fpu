...
void TI2_Config(TIM_TypeDef *TIMx, uint32_t TIM_ICPolarity, uint32_t TIM_ICSelection,
                uint32_t TIM_ICFilter);
...
{
  TIMx->CR1 |= TIM_CR1_TI5E_Msk; // 1<<25
}

/******************************************
 * TIx:TI1/TI2/TI3/TI4/ETR/TI5
...
void TIM2_IntputSourceCfg(uint32_t TIx, uint32_t TIM_IntputSource)
{
  ...
  sysctrl_access();
  SYSCTRL->EDU_CFG8 |= TIM_IntputSource << (TIx * 4); // TIM2 TIx 输入触发源选择
  __dekey();
}
/******************************************
 * TIx:TI1/TI2/TI3/TI4/ETR
...
void TIM2_BreakSourceCfg(uint32_t TIM_BreakSource, uint32_t TIM_Level)
{
  ...
  assert_param(IS_TIM_InputLevel(TIM_Level));
  sysctrl_access();
  SYSCTRL->EDU_CFG9 = TIM_BreakSource; // TIM1 break/break2 输入触发源
  __dekey();
  temprg = SYSCTRL->EDU_CFG9;
  if (temprg & 0x00007F00)
  {
    temprg >>= 8;
    if (TIM_Level == TIM_InputLevel_Low)
    {
      sysctrl_access();
      SYSCTRL->EDU_CFG9 |= temprg; // TIM1 break 输入触发源极性
      __dekey();
    }
  }
  else if (temprg & 0x003F0000)
  {
    ...
    {
      sysctrl_access();
      SYSCTRL->EDU_CFG9 |= temprg; // TIM1 break2 输入触发源极性
      __dekey();
    }
  }
}
...
void TIM2_OCREF_CLR_SourceCfg(uint32_t TIM_OCREF_CLR_Source, uint32_t TIM_Level)
{
  ...
  assert_param(IS_TIM_InputLevel(TIM_Level));
  sysctrl_access();
  SYSCTRL->EDU_CFG9 = TIM_OCREF_CLR_Source; // TIM1 OCREF_CLR 输入触发源
  __dekey();
  temprg = SYSCTRL->EDU_CFG9;
  temprg >>= 24;
  if (TIM_Level == TIM_InputLevel_Low)
  {
    sysctrl_access();
    SYSCTRL->EDU_CFG9 |= temprg; // TIM1 OCREF_CLR 输入触发源极性
    __dekey();
  }
}
/************************************************
...
{
  uint32_t temprg;
  assert_param(IS_TIM3_OCREF_CLR_Source(TIM_OCREF_CLR_Source));
  assert_param(IS_TIM_InputLevel(TIM_Level));
  sysctrl_access();
  SYSCTRL->EDU_CFG5 = TIM_OCREF_CLR_Source; // TIM3 OCREF_CLR 输入触发源
  __dekey();
  temprg = SYSCTRL->EDU_CFG5;
  temprg >>= 8;
  if (TIM_Level == TIM_InputLevel_Low)
...
/*--------------------------------------------------------------------------------------------
TIM 时基初始化  /Time base initialization
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
设置计数器初值  /Set the initial value of the counter
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
获取预分频器值  /Get the predivider value
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
自动重装载使能 缓冲  /Automatic reload enables buffering
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
设置时钟分频  /Set the clock frequency
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
tim 刹车配置初始化 /The brake configuration is initialized
--------------------------------------------------------------------------------------------*/
void TIM_BDTRConfig(TIM_TypeDef *TIMx, TIM_BDTRInitTypeDef *TIM_BDTRInitStruct)
{
  ...
  TIMx->BDTR |= (uint32_t)TIM_BDTRInitStruct->TIM_OSSRState | TIM_BDTRInitStruct->TIM_OSSIState | \
               ...
               TIM_BDTRInitStruct->TIM_Break1Polarity | TIM_BDTRInitStruct->TIM_AutomaticOutput1 | \
              (TIM_BDTRInitStruct->TIM_Break2Polarity << 12) | (TIM_BDTRInitStruct->TIM_AutomaticOutput2 << 12);
  TIMx->BDTR |= TIM_BDTRInitStruct->TIM_Break1 | (TIM_BDTRInitStruct->TIM_Break2 << 12);
}
...
void TIM_BDTRStructInit(TIM_BDTRInitTypeDef *TIM_BDTRInitStruct)
{
  ...
  TIM_BDTRInitStruct->TIM_Break1 = TIM_Break_Disable;
  TIM_BDTRInitStruct->TIM_Break1Polarity = TIM_BreakPolarity_Low;
  TIM_BDTRInitStruct->TIM_AutomaticOutput1 = TIM_AutomaticOutput_Disable;
...
/*--------------------------------------------------------------------------------------------
tim主输出使能  /Enable primary output
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
tim 0C1通道初始化  /0C1 Channel initialization
--------------------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------------------
tim oc2 初始化  /Initialization oc2
--------------------------------------------------------------------------------------------*/
...
void TIM_OC5Init(TIM_TypeDef *TIMx, TIM_OCInitTypeDef *TIM_OCInitStruct)
{
  ...
  tmpccmrx &= (uint32_t)(~((uint32_t)TIM_CCMR3_CC5S));
  /* Select the Output Compare Mode */
  tmpccmrx |= TIM_OCInitStruct->TIM_OCMode;
  ...
  if (TIMx == TIM1)
  {
    ...
    /* Set the Output Idle state */
    tmpcr2 |= (uint32_t)(TIM_OCInitStruct->TIM_OCIdleState << 9);
  }
  /* Write to TIMx CR2 */
  TIMx->CR2 = tmpcr2;
 ...
 * @param  TIMx: where x can be 1, 2 or 3 to select the TIM peripheral.
 * @param  Compare4: specifies the Capture Compare5 register new value.
 * @retval None
 */
...
void TIM_ForcedOC5Config(TIM_TypeDef *TIMx, uint32_t TIM_ForcedAction)
{
  ...
  tmpccmr2 = TIMx->CCMR3;
  /* Reset the OC1M Bits */
  tmpccmr2 &= (uint32_t) ~((uint32_t)TIM_CCMR3_OC5M);
  /* Configure The Forced output Mode */
  tmpccmr2 |= TIM_ForcedAction;
  /* Write to TIMx CCMR2 register */
  TIMx->CCMR3 = tmpccmr2;
...
void TIM_OC5PreloadConfig(TIM_TypeDef *TIMx, uint32_t TIM_OCPreload)
{
  ...
  tmpccmr2 = TIMx->CCMR3;
  /* Reset the OC3PE Bit */
  tmpccmr2 &= (uint32_t) ~((uint32_t)TIM_CCMR3_OC5PE);
  /* Enable or Disable the Output Compare Preload feature */
  tmpccmr2 |= TIM_OCPreload;
  /* Write to TIMx CCMR2 register */
  TIMx->CCMR3 = tmpccmr2;
...
void TIM_OC5FastConfig(TIM_TypeDef *TIMx, uint32_t TIM_OCFast)
{
  ...
  tmpccmr2 = TIMx->CCMR3;
  /* Reset the OC3FE Bit */
  tmpccmr2 &= (uint32_t) ~((uint32_t)TIM_CCMR3_OC5FE);
  /* Enable or Disable the Output Compare Fast Bit */
  tmpccmr2 |= TIM_OCFast;
  /* Write to TIMx CCMR2 */
  TIMx->CCMR3 = tmpccmr2;
...
void TIM_ClearOC5Ref(TIM_TypeDef *TIMx, uint32_t TIM_OCClear)
{
  ...
  tmpccmr2 = TIMx->CCMR3;
  /* Reset the OC3CE Bit */
  tmpccmr2 &= (uint32_t) ~((uint32_t)TIM_CCMR3_OC5CE);
  /* Enable or Disable the Output Compare Clear Bit */
  tmpccmr2 |= TIM_OCClear;
  /* Write to TIMx CCMR2 register */
  TIMx->CCMR3 = tmpccmr2;
...
/**
 * @brief  Configures the TIMx Channel 4N polarity.
 * @param  TIMx: where x can be 1 to select the TIM peripheral.
 * @param  TIM_OCNPolarity: specifies the OC4N Polarity
 *          This parmeter can be one of the following values:
 *            @arg TIM_OCNPolarity_High: Output Compare active high
 *            @arg TIM_OCNPolarity_Low: Output Compare active low
 * @retval None
...
/**
 * @brief  Sets the TIMx Input Capture 5 prescaler.
 * @param  TIMx: where x can be 1, 2 or 3 to select the TIM peripheral.
 * @param  TIM_ICPSC: specifies the Input Capture3 prescaler new value.
 *          This parameter can be one of the following values:
 *            @arg TIM_ICPSC_DIV1: no prescaler
...
void TIM_SetIC5Prescaler(TIM_TypeDef *TIMx, uint32_t TIM_ICPSC)
{
  ...
  /* Reset the IC3PSC Bits */
  TIMx->CCMR3 &= (uint32_t) ~((uint32_t)TIM_CCMR3_IC5PSC);
  /* Set the IC3PSC value */
                       ...
                       uint32_t TIM_ICFilter)
{
  ...
  tmpccer &= (uint32_t) ~((uint32_t)(TIM_CCER_CC4P | TIM_CCER_CC4NP));
...
/**
 * @brief  Configure the TI5 as Input.
 * @param  TIMx: where x can be 1, 2 or 3 to select the TIM peripheral.
 * @param  TIM_ICPolarity: The Input Polarity.
 *          This parameter can be one of the following values:
 *            @arg TIM_ICPolarity_Rising
 ...
 * @param  TIM_ICSelection: specifies the input to be used.
 *          This parameter can be one of the following values:
 *            @arg TIM_ICSelection_DirectTI: TIM Input 5 is selected to be connected to IC5.
 *            @arg TIM_ICSelection_TRC: TIM Input 5 is selected to be connected to TRC.
 * @param  TIM_ICFilter: Specifies the Input Capture Filter.
 *          This parameter must be a value between 0x00 and 0x0F.
 * @retval None
 */
                       ...
                       uint32_t TIM_ICFilter)
{
  ...
  /* Select the Input and set the filter */
  tmpccmr2 &= (uint32_t)((uint32_t)(~(uint32_t)TIM_CCMR3_CC5S) & ((uint32_t) ~((uint32_t)TIM_CCMR3_IC5F)));
  tmpccmr2 |= (uint32_t)(TIM_ICSelection) | (uint32_t)(TIM_ICFilter);
  /* Select the Polarity and set the CC4E Bit */
  tmpccer &= (uint32_t) ~((uint32_t)(TIM_CCER_CC5P | TIM_CCER_CC5NP));
...
void TIM_BreakTrigOci_Cmd(TIM_TypeDef *TIMx, FunctionalState NewState)
{
  if (NewState != DISABLE)
  {
    TIMx->BDTR |= TIM1_BDTR_TRIGOC_Msk;
  }
  else
  {
    TIMx->BDTR &= (uint32_t)(~((uint32_t)TIM1_BDTR_TRIGOC_Msk));
  }
}
