...
inline void ApplyBasicConfig(Lib_Motor::MotorConfig& cfg)
{
    ...
    /* ===================== [3] 相电流采样 ===================== */
    /*
     * current_sensor_type: SHUNT_RESISTOR (三电阻/双电阻采样电阻)
     *                      vs LEM_LA (霍尔电流传感器)
     * current_sense_mode:   DUAL_SENSOR → 两个采样电阻 (Iu, Iv), Iw 计算
     *                       TRIPLE_SENSOR → 三个采样电阻
     ...
     *                               采样值为正才不需 invert
     *
     * phase_shunt_resistor: 相电流采样电阻值 (Ω)
     * phase_amp_gain:       相电流运放增益倍数 (V/V)
     * bus_shunt_resistor:   母线采样电阻值 (Ω)
     * bus_amp_gain:         母线运放增益 (V/V)
     */
    ...
    cfg.sensor.vbus_r_up = 30000.0f;
    cfg.sensor.vbus_r_down = 2000.0f;
     ...
     */
    cfg.sensor.ntc_count = 1;
    cfg.sensor.ntc[0].enabled = true;
    cfg.sensor.ntc[0].topology = Lib_Motor::NtcTopology::HIGH_SIDE_NTC;
    cfg.sensor.ntc[0].r_series = 1000.0f;
    cfg.sensor.ntc[0].r_25 = 10000.0f;
    cfg.sensor.ntc[0].beta = 3290.0f;
    cfg.sensor.ntc[0].over_temp_c = 185.0f;
    ...
    cfg.physical.rated_current = 5.0f;
    ...
    cfg.physical.electrical_param_source = Lib_Motor::MotorPhysicalParamSource::AUTO;
     ...
     */
    cfg.limit.max_current_a = 50.0f;
    cfg.limit.max_speed_rpm = 3000.0f;
    cfg.limit.over_voltage_v = 36.0f;
    cfg.limit.under_voltage_v = 12.0f;
