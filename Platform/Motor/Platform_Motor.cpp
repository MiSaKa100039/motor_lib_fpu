...
Lib_Motor::Result Platform_Motor_Init(void)
{
    ...
    Core_Motor_SetControlFrequencyHz(cfg.control.control_freq_hz);

    const Lib_Motor::Result init_result = Global_Motor_0.init(cfg);
    /* Start TIM/ADC only after the core manager exists; config faults may still be
     * deferred inside the manager so low-level timing remains observable.
     */
