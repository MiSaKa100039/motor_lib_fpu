...
Result MotorAPI::init(const MotorConfig& cfg)
{
    ...
    last_fault_ = MotorConfigCheck::validateBase(cfg, &last_config_fault_detail_);
    if (last_fault_ != Fault::NONE)
    {
        return faultToInitResult(last_fault_);
    }

    MotorConfigFaultDetail deferred_detail = MotorConfigFaultDetail::NONE;
    Fault deferred_fault = MotorConfigCheck::validateStartupTargetMode(cfg, &deferred_detail);
    if (deferred_fault == Fault::NONE && startupModeRequiresFeedback(cfg.control.startup_target_mode))
    {
        deferred_fault = MotorConfigCheck::validateFeedback(cfg, &deferred_detail);
    }

    MotorManager* const mgr = new (managerStorageAt(id_)) MotorManager(cfg);
    g_motor_pool[id_] = mgr;
    mgr->init();
    mgr->setConfigFaultDetail(deferred_detail);

    if (deferred_fault != Fault::NONE)
    {
        Motor_FSM::latchFault(*mgr, deferred_fault);
        last_fault_ = deferred_fault;
        last_config_fault_detail_ = deferred_detail;
        return faultToInitResult(deferred_fault);
    }
