...
enum class MotorConfigFaultDetail : uint16_t
{
    ...
    BUILD_CURRENT_SENSE_MODE_INVALID = 0x0101,
    BUILD_NTC_SLOTS_INVALID = 0x0102,
    BUILD_REDUNDANT_REQUIRES_SENSOR = 0x0103,
    BUILD_OUTPUT_REQUIRES_SENSOR = 0x0104,
    BUILD_REGEN_REQUIRES_ENERGY_FSM = 0x0105,
    BUILD_RESISTOR_REQUIRES_ENERGY_FSM = 0x0106,

    HAL_NULL = 0x0201,
    POLE_PAIRS_INVALID = 0x0202,
    CONTROL_FREQ_INVALID = 0x0203,
    CURRENT_SENSE_MODE_INVALID = 0x0204,
    CURRENT_SENSE_MODE_BUILD_MISMATCH = 0x0205,
    BUS_CURRENT_NOT_BUILT = 0x0206,
    PHASE_VOLTAGE_NOT_BUILT = 0x0207,
    NTC_COUNT_EXCEEDS_BUILD = 0x0208,
    PHASE_CURRENT_MASK_INVALID = 0x0209,
    PHASE_CURRENT_DUAL_MASK_INVALID = 0x020A,
    PHASE_CURRENT_TRIPLE_MASK_INVALID = 0x020B,
    SENSOR_LPF_INVALID = 0x020C,
    SENSOR_LPF_NEGATIVE = 0x020D,
    VBUS_CONFIG_INVALID = 0x020E,
    PHASE_VOLTAGE_CONFIG_INVALID = 0x020F,
    BRAKE_NO_SINK_PATH = 0x0210,           // 允许反向 Iq 但无任何能量承接路径 (无 regen/电阻/low_side)
    BRAKE_ENERGY_FSM_NOT_BUILT = 0x0211,
    BATTERY_REGEN_NOT_BUILT = 0x0212,
    BRAKE_RESISTOR_NOT_BUILT = 0x0213,
    MECHANICAL_BRAKE_NOT_BUILT = 0x0214,
    MECHANICAL_BRAKE_HAL_INVALID = 0x0215,
    BRAKE_CHOPPER_NOT_BUILT = 0x0216,
    BRAKE_CHOPPER_HAL_INVALID = 0x0217,
    BRAKE_CHOPPER_REQUIRES_RESISTOR = 0x0218,
    PHYSICAL_PARAM_INVALID = 0x0219,        // 物理参数来源或固化/辨识值非法

    STARTUP_DEBUG_API_DISABLED = 0x0301,
    STARTUP_IF_DEBUG_DISABLED = 0x0302,
    STARTUP_SPEED_LOOP_DISABLED = 0x0303,
    STARTUP_SENSOR_TORQUE_UNSUPPORTED = 0x0304,
    STARTUP_SENSOR_SPEED_UNSUPPORTED = 0x0305,
    STARTUP_POSITION_UNSUPPORTED = 0x0306,
    STARTUP_MODE_INVALID = 0x0307,

    FEEDBACK_REDUNDANT_NOT_BUILT = 0x0401,
    FEEDBACK_OUTPUT_NOT_BUILT = 0x0402,
    FEEDBACK_STEADY_SOURCE_NOT_BUILT = 0x0403,
    FEEDBACK_STARTUP_SOURCE_NOT_BUILT = 0x0404,
    FEEDBACK_HEALTH_PARAM_INVALID = 0x0405,
    FEEDBACK_REDUNDANT_SENSOR_INVALID = 0x0406,
    FEEDBACK_OUTPUT_SENSOR_INVALID = 0x0407,
    FEEDBACK_REDUNDANT_CHECK_INVALID = 0x0408,
    FEEDBACK_ROTOR_SENSOR_INVALID = 0x0409,
    FEEDBACK_SENSOR_STARTUP_MISMATCH = 0x040A,
    FEEDBACK_ROTOR_CAPABILITY_INVALID = 0x040B,
    FEEDBACK_SENSOR_INVALID_TICKS = 0x040C,
    FEEDBACK_SENSOR_FAULT_ACTION_UNSUPPORTED = 0x040D,
    FEEDBACK_SMO_PARAM_INVALID = 0x040E,
    FEEDBACK_IF_PARAM_INVALID = 0x040F,
    FEEDBACK_HFI_UNAVAILABLE = 0x0410,
    FEEDBACK_SENSOR_TO_SMO_UNAVAILABLE = 0x0411,
    FEEDBACK_STEADY_SOURCE_UNAVAILABLE = 0x0412,
    FEEDBACK_STEADY_SOURCE_INVALID = 0x0413,
    FEEDBACK_HFI_PARAM_INVALID = 0x0414,
    FEEDBACK_SMO_CLOSED_LOOP_DISABLED = 0x0415,
    FEEDBACK_FLYING_START_NOT_BUILT = 0x0416,
    FEEDBACK_FLYING_START_NEEDS_PHASE_VOLTAGE = 0x0417,
    FEEDBACK_FLYING_START_UNAVAILABLE = 0x0418,

    /* 0x04xx 段 — IF 静止保证机制相关 (启动前确认转子不动) */
    IF_STATIONARY_GUARANTEE_REQUIRED        = 0x0419, // IF 启动未配置任何静止保证机制
    IF_STATIONARY_NEEDS_PHASE_VOLTAGE_BUILD = 0x041A, // 选相电压顺逆风启动但 BuildCfg 未编 PHASE_VOLTAGE
    IF_STATIONARY_NEEDS_LOW_SIDE_BRAKE       = 0x041B, // 选低边短路制动但 HAL/brake 配置未就绪
    IF_STATIONARY_EXPLICIT_BYPASS_UNCONFIRMED = 0x041C, // 选明确绕过但 bypass_confirmed 未置真
    FEEDBACK_FLYING_START_SENSOR_STARTUP_UNSUPPORTED = 0x041D, // 有感启动不应开启相电压顺逆风启动

    /* 0x05xx 段 — 控制环带宽 / 参数自推导相关 */
    CURRENT_LOOP_BW_TOO_HIGH          = 0x0501, // 电流环带宽 ≥ 控制频率/10 (离散化失效)
    CURRENT_LOOP_BW_REQUIRES_L_KNOWN = 0x0502, // 自动推导电流环 PI 暂缺 R/L, init 使用 fallback
    SPEED_LOOP_AUTOTUNE_NOT_IMPLEMENTED   = 0x0503, // 速度环自动推导尚未实现 (P3 阶段开放)
    /* [P2] KE_OUT_OF_RANGE 撤回 (改为纯值链, 0=未知; 当前电压限幅不扣减 Ke) */
    RL_IDENTIFY_NOT_BUILT              = 0x0505, // BuildCfg 未编译 AUTO_IDENTIFY: CALIB_RL_IDENTIFY 不可用
    RL_IDENTIFY_FAILED                 = 0x0506, // RL 辨识运行失败 (di/dt 太小 / di/dt 异常)
    KE_IDENTIFY_FAILED                 = 0x0507, // Ke 辨识运行失败 (SMO 未收敛 / speed 过低)
    POSITION_LOOP_AUTOTUNE_NOT_IMPLEMENTED = 0x0508, // 位置环自动推导尚未实现 (P5 阶段开放)
    RL_IDENTIFY_PARAM_INVALID          = 0x0509, // RL 辨识约束参数非法

    /* 0x06xx 段 — 调制 / 控制拓扑相关 */
    MODULATION_SIXSTEP_REQUIRES_TRAP = 0x0601, // SIXSTEP 调制要求 BuildCfg 开 TRAP_CONTROL
    CONTROL_TOPOLOGY_NONE_ENABLED    = 0x0602, // FOC 与 TRAP 至少开一个
};
...
struct MotorMonitorData
{
    ...
    float   temperature[MOTOR_BUILD_NTC_SLOTS];   // 各 NTC 温度 (°C), 索引 0..ntc_count-1
#endif
...
#if MOTOR_BUILD_NTC_SLOTS > 0
    float temp_raw[MOTOR_BUILD_NTC_SLOTS];         // NTC 温度原始(°C)
#endif
};
