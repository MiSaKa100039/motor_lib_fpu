...
enum class CurrentSensorType : uint8_t
{
    ...
    HALL_SENSOR    = 1,  // 霍尔电流传感器 (C6906 等), Vout = VREF + Ip × Sensitivity
    //预留数字采样信号
};
...
struct MotorSensorParam
{
    ...
    float hall_vref_v                 = 2.5f;  // 霍尔零电流输出电压 (V), current_sensor_type=HALL_SENSOR 时生效
    float hall_sensitivity_mV_per_A   = 4.4f;  // 霍尔灵敏度 (mV/A), 如 C6906-300A=4.4
    ...
    uint8_t   ntc_count = 1;    // 实际使用的 NTC 数量 (0~4)
