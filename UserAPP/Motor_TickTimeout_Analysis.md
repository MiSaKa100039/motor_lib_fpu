...
1. `Core_Motor_InjectedAdc1CallbackRateHz` 在空载时接近 `20000`，`cmd` 启动后掉到 `8000` 左右。
...
- `Core_Motor_InjectedAdc1CallbackCount` 不再增长
- LED 翻转逻辑不执行
- `Core_Motor_InjectedAdc1CallbackRateHz` 长时间为 `0`
...
回调恢复后，`Core_Motor_InjectedAdc1CallbackRateHz` 仍然会在 `cmd` 启动后从 `20000` 掉到约 `8000`。这说明问题已经不是“回调不进”，而是“回调进了，但 ISR 处理不过来”。
...
- `Core_Motor_InjectedAdc1CallbackRateHz` 在 RUN 后恢复稳定
- `Core_Motor_TickCoreCyclesLast` / `Core_Motor_TickIsrCyclesLast` 不再长期超预算
- VOFA 发送恢复连续
...
1. `Core_Motor_InjectedAdc1CallbackCount`
2. `Core_Motor_InjectedAdc1CallbackRateHz`
3. `Core_Motor_TickBudgetCycles`
4. `Core_Motor_TickCoreCyclesLast`
5. `Core_Motor_TickIsrCyclesLast`
6. `Core_Motor_TickOverBudgetCount`
